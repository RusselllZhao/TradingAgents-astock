#!/usr/bin/env python3
"""Smoke test for get_concept_blocks data-source contract.
...
EXPECTED_FIELDS = {"ts_code", "trade_date", "name"}
...
def main() -> int:
    ...
    for ticker, case_type in CASES:
        try:
            output = get_concept_blocks(ticker)
...
def validate_output(ticker: str, case_type: str, output: str) -> SmokeResult:
    ...
    if header.get("source") != "Tushare dc_concept_cons":
        notes.append(f"bad_source={header.get('source', 'missing')}")
    if header.get("data_type") != "concept_blocks":
    ...
    if status == "ok":
        ...
        if missing_fields:
            notes.append("missing_dc_concept_cons_fields=" + ",".join(missing_fields))
    elif status == "empty":
        ...
        if "| trade_date |" in output or "theme_code" in output:
            notes.append("invalid_input_returned_concept_table")
