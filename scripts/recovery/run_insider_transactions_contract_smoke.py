#!/usr/bin/env python3
"""Smoke test for get_insider_transactions data-source contract.
...
VALID_STATUSES = {"ok", "partial_data", "empty", "invalid_input", "technical_error"}
...
FORBIDDEN_PATTERNS = {
    ...
    "mootdx": "old_source",
    "stk_managers": "out_of_scope_api",
    "stk_rewards": "out_of_scope_api",
}
...
def main() -> int:
    ...
    from tradingagents.dataflows.a_stock import get_insider_transactions

    results: list[SmokeResult] = []
    for ticker, case_type in CASES:
        try:
            output = get_insider_transactions(ticker)
...
def validate_output(ticker: str, case_type: str, output: str) -> SmokeResult:
    ...
    if header.get("data_type") != "shareholder_f10":
    ...
    if status in {"ok", "partial_data"}:
        if not any(marker in output for marker in EXPECTED_SECTION_MARKERS):
            notes.append("missing_shareholder_sections")
        if "US-style insider transaction data" not in output:
            notes.append("missing_semantic_correction_note")
    elif status == "empty":
        ...
        if "Top 10 Shareholders" in output or "| ts_code |" in output:
            notes.append("invalid_input_returned_shareholder_table")
