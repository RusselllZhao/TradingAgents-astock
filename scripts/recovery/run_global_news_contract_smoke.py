#!/usr/bin/env python3
"""Smoke test for get_global_news data-source contract.
...
VALID_STATUSES = {"ok", "partial_data", "empty", "technical_error"}
...
FORBIDDEN_PATTERNS = {
    ...
    "stock-specific news": "wrong_scope_claim",
    "company announcement": "wrong_data_type_claim",
}

EXPECTED_BODY_FIELDS = {"pub_time", "source", "title"}
RAW_ERROR_HINTS = {
    "requestexception",
    "connectionerror",
    "readtimeout",
...
def validate_output(output: str) -> SmokeResult:
    ...
    if header.get("source") != "CLS + Eastmoney 7x24 + Tushare major_news":
        notes.append(f"bad_source={header.get('source', 'missing')}")
    if header.get("data_type") != "global_news":
        notes.append(f"bad_data_type={header.get('data_type', 'missing')}")
    if header.get("query_target") not in {"market", "macro"}:
    ...
    if status == "technical_error":
        ...
        if raw_error_suppressed != "true":
            notes.append("partial_data_without_raw_suppression")
        if "unavailable_sources=" not in output:
            notes.append("partial_data_without_unavailable_sources_note")
    else:
        if raw_error_suppressed != "false":
            notes.append("non_error_raw_suppression_not_false")

    if status in {"ok", "partial_data"}:
        missing_fields = [field for field in EXPECTED_BODY_FIELDS if field not in output]
        if missing_fields:
            notes.append("missing_news_fields=" + ",".join(missing_fields))
        if (
            "## CLS / Eastmoney Market News" not in output
            and "## Tushare Major News" not in output
        ):
            notes.append("missing_source_sections")
    elif status == "empty":
